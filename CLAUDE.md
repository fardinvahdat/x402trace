...
- **Phase:** v0.3.0 shipped 2026-05-17 ([`x402trace@0.3.0`](https://www.npmjs.com/package/x402trace) — operator-side debugging cut). v0.3.1 hotfix shipping 2026-05-20: two external-contributor PRs ([#66](https://github.com/fardinvahdat/x402trace/pull/66) v2 challenge body parsing by @hypeprinter007-stack, [#67](https://github.com/fardinvahdat/x402trace/pull/67) `bazaar-check` HTTP probe timeouts by @peterxing) + `package.json` description audit-gap fix. Now in **v0.3.2 planning** per [Notion draft](https://www.notion.so/36503c62b26381cfbd1ce4d95fabda82) (metadata-propagation sub-checks D.1/D.2/D.3).
...
- **v0.3.2 candidate scope:** D.1 (manifest hygiene — empty-string detection in `extensions.bazaar`) + D.2 (propagation diff — manifest vs indexer render) + D.3 (indexer-state probe — catches stuck `processing` records). Held: `facilitator-status` (pending 2nd outage cluster), hosted `bazaar-check` demo (Candidate E, 1/4 promotion signals fired).
...
- **Notion v0.3.1+/v0.3.2 plan:** https://www.notion.so/36503c62b26381cfbd1ce4d95fabda82
