...
- **v0.3.2** (2026-05-23) — metadata-propagation cut. D.2 propagation diff (manifest vs CDP discovery render — @zev's api.lastlookdata.com pattern), D.3 indexer-state probe + new `upstream_stuck` verdict (Max's polyodds.bet pattern + the canonical [#2207](https://github.com/x402-foundation/x402/issues/2207) cluster), D.4 `--endpoint <paid-url>` per-route probe (AsaiShota / evanatpizzarobot / 0xdespot's per-route services), D.5 variant-aware `extensions.bazaar` (fixes the [#72](https://github.com/fardinvahdat/x402trace/issues/72) Body-discovery false-positive), public JSON API contract + snapshot tests, facilitator-aware verdict semantics. [ADR-004](./DECISIONS.md). 511 tests.
- **v0.3.3** (2026-05-27) — fast-follow: TateLyman body-discovery copy fix + Bazaar contributor handbook section, TKCollective AgentOracle captured-response fixture (hybrid D.3 + D.5), Giveth funding rail, paid-audits README section, [ADR-007](./DECISIONS.md) drafted for v0.3.4 K (payment-payload echo gap diagnose rules). No verdict-output changes; JSON API contract preserved verbatim.
- **v0.3.4** (current, 2026-05-30) — trust-taxonomy cycle. Four committed items shipped: L (host_pollution listing-hygiene facet, single-voice bypass per [ADR-008](./DECISIONS.md)) + K (payment-payload echo gap rule pair + `upstream_stuck.cause` discriminator per [ADR-007](./DECISIONS.md)) + G (facilitator-fitness per-rail probe with declared-identity-first attribution per [ADR-005](./DECISIONS.md)) + I (`service_unreachable` top-level verdict + first stateful event discriminant + multi-probe consensus + per-cause windows per [ADR-006](./DECISIONS.md)). `bazaar-check` grows from 5 → 8 checks. JSON API X402-44 contract preserved across all four. 4 named external contributors credited (@hypeprinter007-stack / Ferj 🙏, @RipperMercs 🙏, @TKCollective 🙏, @AsaiShota 🙏) + 5 voices behind the design refinements.
- **v0.3.5 scope-eval window opens** — `bazaar-check --batch <urls-file> --concurrency <n> --output-json` for downstream consumers (Cinderwright leaderboard JSON integration spec, 2,777-service target — 1-voice with concrete commitment, held pending cadence answer); v0.3.5 L docs reframe (`self-heals via 30-day window` framing per Ferj's anchor-x402 case + David's CDP-docs anchor); cross-tool-fix-loop-pattern documentation (TomSmart-named pattern, 5 instances).
...
Verify on [Basescan](https://basescan.org/address/0xe922Cc1D44C69bDEc8581Cc9Fd28bab8f59478CC) or [Etherscan](https://etherscan.io/address/0xe922Cc1D44C69bDEc8581Cc9Fd28bab8f59478CC). All amounts welcome — donations fund maintainer time on the next release (currently: v0.3.3 fast-follow + v0.3.4 trust-taxonomy slate per the [v0.3.2 release notes](https://github.com/fardinvahdat/x402trace/releases/tag/v0.3.2)).
